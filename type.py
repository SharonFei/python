Python 3.7.0 (v3.7.0:1bf9cc5093, Jun 27 2018, 04:59:51) [MSC v.1914 64 bit (AMD64)] on win32
Type "copyright", "credits" or "license()" for more information.
>>> numBlocks=int(raw_input('How many blocks of stars do you want?'))
Traceback (most recent call last):
  File "<pyshell#0>", line 1, in <module>
    numBlocks=int(raw_input('How many blocks of stars do you want?'))
NameError: name 'raw_input' is not defined
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
Traceback (most recent call last):
  File "D:/python/Scripts/for.py", line 1, in <module>
    numBlocks=int(raw_input('How many blocks of stars do you want?'))
NameError: name 'raw_input' is not defined
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?5
How many lines in each block?3
Traceback (most recent call last):
  File "D:/python/Scripts/for.py", line 3, in <module>
    numStars=int(raw_input('How many stars per line?'))
NameError: name 'raw_input' is not defined
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?5
How many lines in each block?3
How many stars per line?5
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================3
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
*  
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines in each block?4
How many stars per line?8
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
*
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many stars do you want?5
*
*
*
*
*
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many stars do you want?5
*
*
*
*
*
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many stars do you want?5
*  
*  
*  
*  
*  
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many stars do you want?5
* * * * * 
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many lines of stars do you want?5
How many stars do you want?3
* * * * * * * * * * * * * * * 
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many lines of stars do you want?5
How many stars do you want?3
* * * * * * * * * * * * * * * * * * * * 
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many lines of stars do you want?5
How many stars do you want?3
* * * * * * * * * * * * * * * * 
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many lines of stars do you want?5
How many stars do you want?3
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many lines of stars do you want?3
How many stars do you want?5
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many lines of stars do you want?3
How many stars do you want?5
* * * * * 
* * * * * 
* * * * * 
>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines of stars do you want?4
How many stars do you want?5
* * * * * 
* * * * * 
* * * * * 
* * * * * 

* * * * * 
* * * * * 
* * * * * 
* * * * * 

* * * * * 
* * * * * 
* * * * * 
* * * * * 

>>> 
===================== RESTART: D:/python/Scripts/for.py =====================
How many blocks of stars do you want?3
How many lines of stars do you want?4
How many stars do you want?5
* * * * * 
* * * * * 
* * * * * 
* * * * * 

* * * * * 
* * * * * 
* * * * * 
* * * * * 

* * * * * 
* * * * * 
* * * * * 
* * * * * 

>>> a=24
>>> b=float(a)
>>> print(b)
24.0
>>> a
24
>>> c=38.0
>>> d=int(c)
>>> d
38
>>> c
38.0
>>> 0.1+0.2
0.30000000000000004
>>> print(0.1+0.2)
0.30000000000000004
>>> print 0.1+0.2
SyntaxError: Missing parentheses in call to 'print'. Did you mean print(0.1+0.2)?
>>> print(0.1+0.2)
0.30000000000000004
>>> f=0.1+0.2
>>> f
0.30000000000000004
>>> print (f)
0.30000000000000004
>>> f=int(0.1+0.2)
>>> f
0
>>> f=float(0.1+0.2)
>>> f
0.30000000000000004
>>> e=54.99
>>> f=int(e)
>>> e
54.99
>>> f
54
>>> a='76.3'
>>> b=float(a)
>>> a
'76.3'
>>> b
76.3
>>> type(a)
<class 'str'>
>>> type(b)
<class 'float'>
>>> type(e)
<class 'float'>
>>> print float('fred')
SyntaxError: invalid syntax
>>> cel=5.0/9*(fahr-32)
Traceback (most recent call last):
  File "<pyshell#32>", line 1, in <module>
    cel=5.0/9*(fahr-32)
NameError: name 'fahr' is not defined
>>> int(13.7)
13
>>> int(13.7)+1
14
>>> int(13.2)+1
14
>>> a=float('12.34')
>>> a
12.34
>>> a=float(12.34)
>>> a
12.34
>>> int (56.78)
56
>>> a=int('9.0')
Traceback (most recent call last):
  File "<pyshell#41>", line 1, in <module>
    a=int('9.0')
ValueError: invalid literal for int() with base 10: '9.0'
>>> a=int(9.0)
>>> a
9
>>> a=int('75')
>>> a
75
>>> a=int('7.5')
Traceback (most recent call last):
  File "<pyshell#46>", line 1, in <module>
    a=int('7.5')
ValueError: invalid literal for int() with base 10: '7.5'
>>> a=int(8.3)
>>> a
8
>>> 
